...
Scalar VisionEnv::computeCollisionApproachPenalty() {
  ...
  for (size_t sort_idx : sort_indexes(relative_pos_norm_)) {
    ...
    const Scalar dist_margin = 1;  // 0.5
  ...
  if (quad_state_.v.norm() > 10)
    collision_penalty *= log(1 + quad_state_.v.norm()) * collision_coeff_ /
                         (total_detectable_obstacles + 1);
  else
    collision_penalty = 0;

  logger_.info("AAA" + to_string(collision_penalty));


  return collision_penalty;
}
Scalar VisionEnv::computeXprogressReward() {
  return goal_dist_rew_ *
         (1.0 - sqrt(abs(goal_pos_[0] - quad_state_.p(QS::POSX)) /
                     abs(max_dist_[0])));
}
Scalar VisionEnv::computeGoalApproachReward() {
  // distance traveled towards goal, normalized in [0,1
  if (quad_state_.p(QS::POSX) > xMax) {
    Scalar a = quad_state_.p(QS::POSX) - xMax;
    xMax = quad_state_.p(QS::POSX);
    return a;
  } else
    return 0;
...
Scalar VisionEnv::computeCamOrientationReward() {
  ...
  string gg = " ";
  // if "global" the target dir is goal, if "local" the target dir is the drone
  // dir
...

  {
    ...
    Eigen::Matrix3d qx_rot_matrix = q.normalized().toRotationMatrix(); //compute
    rot matrix Eigen::Vector3d origin(1, 0, 0); //initial camera orientation
  ...
  // dot product= 1 if same dir,-1 if opposite dir, 0 if dirs normal to each
  // other
  // return (target_dir.dot(camera_dir) + 1) / 2 -1; //normalized in interval
  // [-1,0]
...
Scalar VisionEnv::multiSummedComponentsReward() {
  ...
  Scalar collision_weight = 0;  // for approaching or colliding with the
                                // obstacle
  ...
  Scalar attitude_weight =
    1;  // for orienting the camera in the target direction
  Scalar survive_weight = 1;  // for surviving the step


  // - compute approach penalty: from 0 if far from the obstacle, to 1 if
  // collides
  Scalar collision_penalty =
    computeCollisionApproachPenalty() * collision_weight;
...
Scalar VisionEnv::camAndXBasedReward() {
  ...
  if (quad_state_.p(QS::POSX) <=
      quad_old_state_.p(QS::POSX)) {  // se non ho fatto passo verso x
    total_reward = 0;                 // non do reward per nulla
  } else {
    if (attitude_reward > 0) {
      total_reward = dist_reward * attitude_reward;
    } else {
      total_reward = 0;
    }
  }
...
Scalar VisionEnv::newReward() {
  ...
  Scalar attitude_weight =
    2;  // for orienting the camera in the target direction
  Scalar ang_vel_weight = 1;
  // - go towards goal reward
  Scalar dist_reward = computeGoalApproachReward() * distance_weight;


  Scalar sphere_collision = computeCollisionApproachPenalty();
  ...
  if (attitude_reward > 0.8 && dist_reward > 0) {
    Scalar val = attitude_reward;
    val = abs((val - 0.8)) * 5;
    attitude_reward *= attitude_weight;
    logger_.info("A" + to_string(dist_reward * val));
    return dist_reward * val + dist_reward + sphere_collision;
  } else {
    return 0;
  }
}
...
bool VisionEnv::isTerminalState(Scalar &reward) {
  ...
  if (!x_valid || !y_valid || !z_valid) {
    ...
    // << quad_state_.p(QS::POSX) << " <= " << world_box_[1] -
    // safty_threshold << " ?\n";
    // }
    //
    // if(!y_valid){
    ...
    // << quad_state_.p(QS::POSY) << " <= " << world_box_[3] -
    // safty_threshold << " ?\n";
    // }
    //
    // if(!z_valid){
    ...
    // << quad_state_.p(QS::POSZ) << " <= " << world_box_[5] -
    // safty_threshold << " ?\n";
    // }
...
std::ostream &operator<<(std::ostream &os, const VisionEnv &vision_env) {
  ...
  os << "Vision Environment:\n"
     ...
     << "obs_std = [" << vision_env.obs_std_.transpose() << "]" << std::endl;
  os.precision();
  return os;
}
